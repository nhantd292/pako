...
class TeacherTable extends AbstractTableGateway implements ServiceLocatorAwareInterface {
	...
	public function countItem($arrParam = null, $options = null){
	    ...
	    if($options['task'] == 'list-item') {
	        $result	= $this->tableGateway->select(function (Select $select) use ($arrParam){
	            ...
	            $select -> where -> equalTo('company_position_id', '1534219500200828a7to4w');
	...
	public function listItem($arrParam = null, $options = null){
	    ...
	    if($options['task'] == 'cache-status') {
	        $cache = $this->getServiceLocator()->get('cache');
	        $cache_key = 'AdminUserTeacherStatus';
	        $result = $cache->getItem($cache_key);
	        if (empty($result)) {
	            $items	= $this->tableGateway->select(function (Select $select) use ($arrParam){
	                $select -> order(TABLE_USER .'.name ASC')
	                        -> where -> equalTo('company_position_id', '1534219500200828a7to4w')
