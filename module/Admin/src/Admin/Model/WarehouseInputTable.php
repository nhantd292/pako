...
class WarehouseInputTable extends DefaultTable {
	    ...
	    if($options['task'] == 'list-item') {
	        $result	= $this->tableGateway->select(function (Select $select) use ($arrParam, $options){
                ...
                $select -> join(TABLE_CONTACT, TABLE_CONTACT .'.id = '. TABLE_WAREHOUSE_INPUT .'.customer_id', array( 'customer_name' => 'name'), 'inner')
                    -> join(TABLE_WAREHOUSE, TABLE_WAREHOUSE .'.id = '. TABLE_WAREHOUSE_INPUT .'.inventory_id', array( 'warehouse_name' => 'name'), 'inner');

                if(!empty($ssFilter['filter_date_begin']) && !empty($ssFilter['filter_date_end'])) {
                    $select -> where -> NEST
                        -> greaterThanOrEqualTo(TABLE_WAREHOUSE_INPUT .'.created', $date->formatToData($ssFilter['filter_date_begin']))
                        ->AND
                        -> lessThanOrEqualTo(TABLE_WAREHOUSE_INPUT .'.created', $date->formatToData($ssFilter['filter_date_end']. ' 23:59:59') )
                        -> UNNEST;
                } elseif (!empty($ssFilter['filter_date_begin'])) {
                    $select->where->greaterThanOrEqualTo(TABLE_WAREHOUSE_INPUT .'.created', $date->formatToData($ssFilter['filter_date_begin']));
                } elseif (!empty($ssFilter['filter_date_end'])) {
                    $select->where->lessThanOrEqualTo(TABLE_WAREHOUSE_INPUT .'.created', $date->formatToData($ssFilter['filter_date_end']. ' 23:59:59') );
                }
                ...
                if(isset($ssFilter['filter_keyword']) && $ssFilter['filter_keyword'] != '') {
                    $select->where->NEST
                        ->like(TABLE_WAREHOUSE_INPUT .'.code', '%'.$ssFilter['filter_keyword'].'%')
                        ->UNNEST;
                }
            })->count();
	    }
	...
	public function listItem($arrParam = null, $options = null){
		...
		if($options['task'] == 'list-item') {
			$result	= $this->tableGateway->select(function (Select $select) use ($arrParam, $options){
                ...
                $select -> join(TABLE_CONTACT, TABLE_CONTACT .'.id = '. TABLE_WAREHOUSE_INPUT .'.customer_id', array( 'customer_name' => 'name', 'customer_phone' => 'phone'), 'inner')
                    -> join(TABLE_WAREHOUSE, TABLE_WAREHOUSE .'.id = '. TABLE_WAREHOUSE_INPUT .'.inventory_id', array( 'warehouse_name' => 'name'), 'inner');
	...
	public function getItem($arrParam = null, $options = null){
		...
		if($options == null) {
			$result	= $this->tableGateway->select(function (Select $select) use ($arrParam, $options){
                $select -> where -> equalTo(TABLE_WAREHOUSE_INPUT.'.id', $arrParam['id']);
    		})->current();
		}

        if($options['task'] == 'full') {
            $result	= $this->tableGateway->select(function (Select $select) use ($arrParam, $options){
                $select -> join(TABLE_CONTACT, TABLE_CONTACT .'.id = '. TABLE_WAREHOUSE_INPUT .'.customer_id', array( 'name','phone','customer_type_id'), 'inner')
                    -> join(TABLE_WAREHOUSE, TABLE_WAREHOUSE .'.id = '. TABLE_WAREHOUSE_INPUT .'.inventory_id', array( 'warehouse_name' => 'name'), 'inner')
                    -> join(TABLE_CUSTOMER_DEBT, TABLE_CUSTOMER_DEBT .'.warehouse_input_id = '. TABLE_WAREHOUSE_INPUT .'.id', array('old_debt', 'new_debt'), 'inner');
	...
	public function saveItem($arrParam = null, $options = null){
        ...
        if ($options['task'] == 'update-code') {
            ...
            $code = $action->createCode("PNH", $item->index);
            $data = array(
                'code' => $code,
            );
            try {
	    ...
	    if($options['task'] == 'add-item') {
            ...
            try {
                ...
                throw new \Exception('Insert warehouse input Table failed: ' . $e->getMessage());
            }
	    }
