...
class ContractCodTable extends DefaultTable {
    ...
    public function countItem($arrParam = null, $options = null){
	    if($options['task'] == 'list-item') {
	        $result	= $this->tableGateway->select(function (Select $select) use ($arrParam, $options){
                ...
                $select -> join(TABLE_CONTRACT, TABLE_CONTRACT .'.id = '. TABLE_CONTRACT_COD .'.contract_id',
                    array(
                        'contract_code' => 'code',
                    ), 'inner');

                if(!empty($ssFilter['filter_date'])) {
                    $select -> where -> equalTo(TABLE_CONTRACT_COD .'.date', $ssFilter['filter_date']);
                }
                if(!empty($ssFilter['filter_contract_id'])) {
                    $select -> where -> equalTo(TABLE_CONTRACT_COD .'.contract_id', $ssFilter['filter_contract_id']);
                }
