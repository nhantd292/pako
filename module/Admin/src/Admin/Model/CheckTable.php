...
class CheckTable extends DefaultTable {
	    ...
	    if($options['task'] == 'list-item') {
	        $result	= $this->tableGateway->select(function (Select $select) use ($arrParam, $options){
                ...
                if(isset($ssFilter['filter_keyword']) && $ssFilter['filter_keyword'] != '') {
                    ...
                    if(strlen($number->formatToPhone($filter_keyword)) >= 10) {
                        ...
                        $select -> where -> NEST
                            ...
                            ->Or
                            -> like(TABLE_CONTACT. '.email', '%'. $filter_keyword .'%')
                            ->Or
    			...
    			if(!empty($ssFilter['filter_date_begin']) && !empty($ssFilter['filter_date_end'])) {
    			    $select -> where -> NEST
                    			     -> greaterThanOrEqualTo(TABLE_CONTRACT .'.production_date_send', $date->formatToSearch($ssFilter['filter_date_begin']))
                    			     ->AND
                    			     -> lessThanOrEqualTo(TABLE_CONTRACT .'.production_date_send', $date->formatToSearch($ssFilter['filter_date_end']) . ' 23:59:59')
                    			     -> UNNEST;
    			} elseif (!empty($ssFilter['filter_date_begin'])) {
    			    $select->where->greaterThanOrEqualTo(TABLE_CONTRACT .'.production_date_send', $date->formatToSearch($ssFilter['filter_date_begin']));
    			} elseif (!empty($ssFilter['filter_date_end'])) {
    			    $select->where->lessThanOrEqualTo(TABLE_CONTRACT .'.production_date_send', $date->formatToSearch($ssFilter['filter_date_end']) . ' 23:59:59');
    			}
    			...
    			if(!empty($ssFilter['filter_product_type_id'])) {
    				$select -> where -> equalTo(TABLE_CONTRACT .'.production_type_id', $ssFilter['filter_product_type_id']);
    			}
	...
	public function saveItem($arrParam = null, $options = null){
        ...
        if($options['task'] == 'import-update') {
            ...
            if(!empty($arrData['bill_code'])){
                $data['bill_code'] = $arrData['bill_code'];
            }
...
//            );
        ...
        // Cập nhật nhân viên giao vận mới - chỉ sử dụng một lần
//        if($options['task'] == 'update_shipper_new') {
//            echo "<pre>";
//            print_r($arrData);
//            echo "</pre>";
//            foreach($arrData as $key => $value){
//                if(!empty($value)){
//                    $sql_update = "UPDATE ".TABLE_CONTRACT." SET ".TABLE_CONTRACT.".shipper_id = '".$value."' WHERE ".TABLE_CONTRACT.".shipper_id = '".$key."'";
//                    echo "<pre>";
//                    print_r($sql_update);
//                    echo "</pre>";
//                }
