...
class CarpetColorController extends ActionController {
    
    public function init() {
        
        // Thiết lập options
        $this->_options['tableName'] = 'Admin\Model\CarpetColorTable';
        $this->_options['formName'] = 'formAdminCarpetColor';
    ...
    public function indexAction() {
        $myForm	= new \Admin\Form\Search\CarpetColor($this->getServiceLocator(), $this->_params['ssFilter']);
        ...
        $this->_viewModel['caption']                = 'Màu thảm - Danh sách';
    ...
    public function addAction() {
        $myForm = $this->getForm();
        if($this->getRequest()->isPost()) {
            $myForm->setInputFilter(new \Admin\Filter\CarpetColor(array('data' => $this->_params['data'], 'route' => $this->_params['route'])));
        ...
        $this->_viewModel['myForm']	    = $myForm;
        $this->_viewModel['caption']    = 'Màu thảm - Thêm mới';
        return new ViewModel($this->_viewModel);
    }
    ...
    public function editAction() {
        ...
        $myForm = new \Admin\Form\CarpetColor\Edit($this->getServiceLocator(), $this->_params);
        
        if(!empty($this->_params['data']['id'])) {
            $color = $this->getServiceLocator()->get('Admin\Model\CarpetColorTable')->getItem(array('id' => $this->_params['data']['id']));         
        ...
        if($this->getRequest()->isPost()){
            if($this->_params['data']['modal'] == 'success') {
                $myForm->setInputFilter(new \Admin\Filter\CarpetColor\Edit($this->_params));
                $myForm->setData($this->_params['data']);
    
                if($myForm->isValid()){
                    $this->_params['data'] = $myForm->getData(FormInterface::VALUES_AS_ARRAY);
                    $this->_params['item'] = $color;
                    $result = $this->getServiceLocator()->get('Admin\Model\CarpetColorTable')->saveItem($this->_params, array('task' => 'edit-item'));
        ...
        $this->_viewModel['myForm']         = $myForm;
        $this->_viewModel['caption']        = 'Sửa Màu thảm';
