...
class TeacherController extends ActionController {
    ...
    public function init() {
        // Thiết lập options
        $this->_options['tableName'] = 'Admin\Model\TeacherTable';
        $this->_options['formName'] = 'formAdminTeacher';
    ...
    public function indexAction() {
        ...
        $edu_class_teacher = $this->getServiceLocator()->get('Admin\Model\EduClassTable')->listItem(null, array('task' => 'public'));
        $edu_class = array();
        foreach ($edu_class_teacher AS $key => $val) {
            if(!empty($val['teacher_ids'])) {
                $teacher_ids = explode(',', $val['teacher_ids']);
                foreach ($teacher_ids AS $teacher_id) {
                    $edu_class[$teacher_id][] = $val['time'] .' - '. $val['schedule'] .' - '. $val['name'] .' - Kết thúc: '. $dateFormat->formatToView($val['end_date']);
                }
            }
        }
        ...
        $this->_viewModel['caption']            = 'Giáo viên - Danh sách';
