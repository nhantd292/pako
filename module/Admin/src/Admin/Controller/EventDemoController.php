...
class EventDemoController extends ActionController {
    
    public function init() {
        
        // Thiết lập options
        $this->_options['tableName'] = 'Admin\Model\EventTable';
        $this->_options['formName'] = 'formAdminEventDemo';
    ...
    public function indexAction() {
        $myForm	= new \Admin\Form\Search\EventDemo($this->getServiceLocator());
        $myForm->setData($this->_params['ssFilter']);
        
        $items = $this->getTable()->listItem($this->_params, array('task' => 'list-item', 'type' => 'demo'));
        
        $this->_viewModel['myForm']	    	= $myForm;
        $this->_viewModel['items']      	= $items;
        $this->_viewModel['count']      	= $this->getTable()->countItem($this->_params, array('task' => 'list-item', 'type' => 'demo'));
        ...
        $this->_viewModel['caption']    	= 'Demo - Danh sách';
    ...
    public function formAction() {
        ...
        $task = 'add-item';
        $caption = 'Demo - Thêm mới';
        $item = array();
        if(!empty($this->params('id'))) {
            $this->_params['data']['id'] 	= $this->params('id');
            $item = $this->getTable()->getItem($this->_params['data']);
            $item['teacher_ids']			= !empty($item['teacher_ids']) ? unserialize($item['teacher_ids']) : array();
            $item['coach_ids']				= !empty($item['coach_ids']) ? unserialize($item['coach_ids']) : array();
            $item['public_date']			= $dateFormat->formatToView($item['public_date']);
            if(!empty($item)) {
                $myForm->setInputFilter(new \Admin\Filter\EventDemo(array('id' => $this->_params['data']['id'])));
                $myForm->bind($item);
                $task = 'edit-item';
                $caption = 'Demo - Sửa';
            }
    ...
    public function exportAction() {
        ...
        $events             = $this->getTable()->listItem($this->_params, array('task' => 'list-all', 'type' => 'demo'));
        ...
        $arrData = array(
            ...
            array('field' => 'mc_ids', 'title' => 'MC', 'type' => 'data_source', 'data_source' => $user, 'data_source_field' => 'fullname'),
