            ...
            if (in_array($item['state'], array(COMPLETE_STATUS, CANCEL_STATUS))) {
                ...
                if ($control_action == COMPLETE_STATUS) {
                    if(!in_array(SYSTEM, $permission_ids) && !in_array(ADMIN, $permission_ids) && !in_array(ACCOUNTING, $permission_ids)){
                        $this->flashMessenger()->addErrorMessage('Bạn không có quyền hoàn thành đơn hàng!');
                    }
                    else{
                        ##### begin #####
                        $connection->beginTransaction();
        ...
        if($this->getRequest()->isPost()){
            ...
            if($myForm->isValid()){
                ...
                if($check_emty_data){
                    ...
                    # Cập nhật đơn hàng
                    $contract_id = $this->getTable()->saveItem($this->_params, array('task' => 'edit-kov-item'));
...
//                    $price_total    = $number->formatToData($this->_params['data']['price_total']);
    ...
    public function exportVATAction() {
        ...
        // Cấu hình mảng dữ liệu: Đã đổi Ngày hóa đơn sang format_code => '@' (Text)
        $arrData = array(
            array('field' => 'stt', 'title' => 'Số thứ tự hóa đơn (*)', 'align' => 'center', 'format_code' => 'General'),
            array('field' => 'contract_date', 'title' => 'Ngày hóa đơn', 'type' => 'date', 'align' => 'center', 'format_code' => '@'), // Để dạng Text cứng
            array('field' => 'company_name', 'title' => 'Tên khách hàng', 'format_code' => '@'),
            array('field' => 'company_address', 'title' => 'Địa chỉ', 'format_code' => '@'),
            array('field' => 'company_mst', 'title' => 'Mã số thuế', 'align' => 'center', 'format_code' => '@'), // Để dạng Text cứng giữ số 0
...
//            $item['percent_vat'] = $this->_params['settings']['General.System.Vat']['value'];
            ...
            foreach ($arrData AS $key => $data) {
                ...
                switch ($data['type']) {
                    case 'date':
                        $formatDate = $data['format'] ? $data['format'] : 'd/m/Y';
                        // Định dạng chuỗi ngày d/m/Y sạch sẽ, nếu dòng trống do gộp thì giữ rỗng ''
                ...
                // Ép kiểu số float đối với các trường số (loại trừ các trường có format_code là Text @ hoặc General)
                if (isset($data['format_code']) && $data['format_code'] !== '@' && $data['format_code'] !== 'General' && $value !== '') {
                    $value = (float)$value;
                }

                // Ghi dữ liệu vào ô (Nếu cấu hình format_code là '@' thì ép kiểu Text chuỗi Explicit để bảo toàn dữ liệu gốc)
                ...
                // Áp dụng mã định dạng format_code vào ô Excel
                if (isset($data['format_code']) && $value !== '') {
                    $cellStyle->getNumberFormat()->setFormatCode($data['format_code']);
                }
        ...
        // Tự động giãn chiều rộng cột (Auto Fit)
    ...
    public function exportPOSv1Action() {
        ...
        $arrData = array(
            array('field' => 'stt', 'title' => 'Số thứ tự hóa đơn (*)', 'align' => 'center'),
            array('field' => 'contract_date', 'title' => 'Ngày hóa đơn', 'type' => 'date', 'align' => 'center'),
            array('field' => 'company_name', 'title' => 'Tên đơn vị mua hàng'),
            array('field' => 'company_mst', 'title' => 'Mã số thuế', 'align' => 'center'),
            array('field' => 'company_address', 'title' => 'Địa chỉ'),
            array('field' => 'customer_name', 'title' => 'Người mua hàng'),
            array('field' => 'company_email', 'title' => 'Email'),
            array('field' => 'customer_phone', 'title' => 'Số điện thoại'),
            array('field' => 'customer_cccd', 'title' => 'Căn cước công dân'),
            array('field' => 'pay_type', 'title' => 'Hình thức thanh toán (*)', 'align' => 'center'),
...
//        $this->_viewModel['caption']                    = 'Chi tiết - '.$this->caption;
//        $viewModel = new ViewModel($this->_viewModel);
//        $viewModel->setTerminal(true);
//
//        return $viewModel;
