...
class ApiController extends ActionController
{
    ...
    public function loadProductsWarehouseOutputAction()
    {
        $itemPerpage = 20;
        $curentPage = $this->_params['data']['curentPage'] ? $this->_params['data']['curentPage'] : 1;
        $paginator = array(
        ...
        $ssFilter = array(
            'filter_customer_id' => $this->_params['data']['filter_customer_id'] ? $this->_params['data']['filter_customer_id'] : 'customer_id',
            'filter_state' => 'complete',
            'filter_numbers_return' => 1,
        );
        ...
        $this->_viewModel['kovProducts'] = $this->getServiceLocator()->get('Admin\Model\WarehouseInputDetailTable')->listItem($param, array('task' => 'list-item'));
        $this->_viewModel['count'] = $this->getServiceLocator()->get('Admin\Model\WarehouseInputDetailTable')->countItem($param, array('task' => 'list-item'));
        $this->_viewModel['itemPerpage'] = $itemPerpage;
        $this->_viewModel['curentPage'] = $curentPage;
        $this->_viewModel['contactId'] = $this->_params['data']['filter_contact_id'];
    ...
    public function addProductWarehouseOutputToListAction()
    {
        if ($this->_params['data']['id']) {
            $product = $this->getServiceLocator()->get('Admin\Model\WarehouseInputDetailTable')->getItem($this->_params['data'], array('task' => 'search'));
            $this->_viewModel['product'] = (array)$product;
            $this->_viewModel['data'] = $this->_params['data'];
            $viewModel = new ViewModel($this->_viewModel);
            $viewModel->setTerminal(true);
    ...
    public function loadProductsWarehouseRotationAction()
    {
        $itemPerpage = 20;
        $curentPage = $this->_params['data']['curentPage'] ? $this->_params['data']['curentPage'] : 1;
        $paginator = array(
    ...
    public function addProductToWarehouseRotationAction()
    {
        if ($this->_params['data']['id']) {
            $product = $this->getServiceLocator()->get('Admin\Model\ProductsInventoryTable')->getItem($this->_params['data'], array('task' => 'filter'));
            $this->_viewModel['product'] = (array)$product;
    ...
    public function listProductWarehouseOutputAction()
    {
        if ($this->getRequest()->isXmlHttpRequest()) {
            $warehouse_output_id = $this->_params['data']['warehouse_output_id'];
            $items = $this->getServiceLocator()->get('Admin\Model\WarehouseOutputDetailTable')->listItem(array('warehouse_output_id' => $warehouse_output_id), array('task' => 'list-ajax'));
            $this->_viewModel['items'] = $items;
            $viewModel = new ViewModel($this->_viewModel);
            $viewModel->setTerminal(true);
        } else {
    ...
    public function listProductWarehouseRotationAction()
    {
        if ($this->getRequest()->isXmlHttpRequest()) {
            $warehouse_rotation_id = $this->_params['data']['warehouse_rotation_id'];
            $items = $this->getServiceLocator()->get('Admin\Model\WarehouseRotationDetailTable')->listItem(array('warehouse_rotation_id' => $warehouse_rotation_id), array('task' => 'list-ajax'));
            $this->_viewModel['items'] = $items;
            $viewModel = new ViewModel($this->_viewModel);
            $viewModel->setTerminal(true);
        } else {
...
//         $data_post =  json_decode($post_data_test, true);
//        $this->postJson(file_get_contents('php://input'));// Đẩy dữ liệu sang webhook.site để kiểm tra
    public function updateKovOrdersAction()
    {
        ...
        $kovOrdersTable = $this->getServiceLocator()->get('Admin\Model\KovOrdersTable');
        $updated = 0;
        $created = 0;
        $customerCache = array();

        try {
            foreach ($data_post['Notifications'] as $notification) {
                if (empty($notification['Data'])) continue;

                foreach ($notification['Data'] as $item) {

                    // Kiểm tra đơn hàng đã tồn tại chưa
                    $existingOrder = $kovOrdersTable->getItem(array('Id' => $item['Id']));

                    // Lấy thông tin khách hàng (cache để tránh gọi API trùng)
                    if (!isset($customerCache[$item['CustomerId']])) {
                        $customerRes = $this->kiotviet_call(RETAILER, $this->kiotviet_token, '/customers/' . $item['CustomerId']);
                        $customerData = json_decode($customerRes, true);

                        $customerPhone = '';
                        if (is_array($customerData) && isset($customerData['contactNumber'])) {
                            $customerPhone = $customerData['contactNumber'];
                        }

                        $customerCache[$item['CustomerId']] = $customerPhone;
                    } else {
                    ...
                    if ($customerPhone != '') {
                        $list_order = $kovOrdersTable->listItem(
                            array('ssFilter' =>
                                array(
                                    'filter_CustomerPhone' => $customerPhone,
                                    'order_type' => 'ASC',
//                                    'filter_status_array' => [1]
                                )
                            ),
                            array('task' => 'list-item')
                        );

                        print_r(json_encode($list_order->toArray()));
                        exit;
                    }
                    ...
                    $kovOrdersTable->saveItem(array('data' => $data), array('task' => $task));
                }
            }
        ...
        try {
            ...
            foreach ($dataPost['Notifications'] as $notification) {
                ...
                foreach ($notification['Data'] as $item) {
                    ...
                    // Lấy thông tin từ DB
                    $invoice = $invoiceTable->getItem(['Id' => $item['Id']]);
