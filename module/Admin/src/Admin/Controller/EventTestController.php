...
class EventTestController extends ActionController {
    
    public function init() {
        
        // Thiết lập options
        $this->_options['tableName'] = 'Admin\Model\EventTable';
        $this->_options['formName'] = 'formAdminEventTest';
    ...
    public function indexAction() {
        $myForm	= new \Admin\Form\Search\EventTest($this->getServiceLocator());
        $myForm->setData($this->_params['ssFilter']);
        
        $items = $this->getTable()->listItem($this->_params, array('task' => 'list-item', 'type' => 'test'));
        ...
        $this->_viewModel['training_room']	= $this->getServiceLocator()->get('Admin\Model\DocumentTable')->listItem(array( "table" => "document", "where" => array( "code" => "training-room" ), "order" => array("ordering" => "ASC", "name" => "ASC"), "view"  => array( "key" => "id", "value" => "name", "sprintf" => "%s" ) ), array('task' => 'cache'));
        $this->_viewModel['caption']    	= 'Test - Danh sách';
    ...
    public function formAction() {
        ...
        $task = 'add-item';
        $caption = 'Test - Thêm mới';
        $item = array();
        if(!empty($this->params('id'))) {
            $this->_params['data']['id']	= $this->params('id');
            $item = $this->getTable()->getItem($this->_params['data']);
            $item['teacher_ids']			= unserialize($item['teacher_ids']);
            if(!empty($item)) {
                $myForm->setInputFilter(new \Admin\Filter\EventTest(array('id' => $this->_params['data']['id'])));
                $myForm->bind($item);
                $task = 'edit-item';
                $caption = 'Test - Sửa';
            }
    ...
    public function exportAction() {
        ...
        $events             = $this->getTable()->listItem($this->_params, array('task' => 'list-all', 'type' => 'test'));
