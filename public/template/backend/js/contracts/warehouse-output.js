...
var data = {filter_products_type: $(`select[name="filter_products_type"]`).val(), filter_keyword: $(`input[name="filter_keyword"]`).val(), filter_customer_id: $(`input[name="customer_id"]`).val()};
load_action('#loadProducts', '/xadmin/api/loadProductsWarehouseOutput/', data);
...
function updateTotal() {
    ...
    $.each($('.list-product-contract tr'), function (index, value) {
        ...
        var number = $(this).find('.quantity input').val() ? $(this).find('.quantity input').val() : 0;
