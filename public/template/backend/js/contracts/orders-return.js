...
var is_post = parseInt($('#is_post').text().trim());
if (contactId && is_post == 0) {
    checkContactToElement(contactId, 'element');
}

var data = {filter_products_type: $(`select[name="filter_products_type"]`).val(), filter_keyword: $(`input[name="filter_keyword"]`).val(), filter_contact_id: contactId};
load_action('#loadProducts', '/xadmin/api/loadKovProductsReturn/', data);
