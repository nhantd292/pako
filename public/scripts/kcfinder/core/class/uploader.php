...
/** Settings which can override default settings if exists as keys in $config['types'][$type] array
  * @var array */
    protected $typeSettings = array('disabled', 'theme', 'dirPerms', 'filePerms', 'denyZipDownload', 'maxImageWidth', 'maxImageHeight', 'thumbWidth', 'thumbHeight', 'mediumWidth', 'mediumHeight', 'jpegQuality', 'access', 'filenameChangeChars', 'dirnameChangeChars', 'denyExtensionRename', 'deniedExts', 'watermark');
...
/** Magic method which allows read-only access to protected or private class properties
  ...
  * @return mixed */
    ...
    protected function makeThumb($file, $overwrite=true) {
        $imgThumb = image::factory($this->imageDriver, $file);
        $imgMedium = image::factory($this->imageDriver, $file);

        // Drop files which are not images
        if ($imgThumb->initError)
            return true;
        ...
        $medium = substr($file, strlen($this->config['uploadDir']));
        $medium = $this->config['uploadDir'] . "/" . $this->config['mediumDir'] . "/" . $medium;
        $medium = path::normalize($medium);
        $mediumDir = dirname($medium);
        if (!is_dir($mediumDir) && !@mkdir($mediumDir, $this->config['dirPerms'], true))
            return false;
        
        if (!$overwrite && is_file($mediumDir))
            return true;
        ...
        // Save medium
        if (($imgMedium->width <= $this->config['thumbWidth']) && ($imgMedium->height <= $this->config['thumbHeight']) ) {
            if (in_array($type, array("gif", "jpeg", "png"))) {
                return true;
            }
        } else {
            if($this->config['resizeType'] == 'crop') {
                if (!$imgMedium->resizeCrop($this->config['mediumWidth'], $this->config['mediumHeight'])) {
                    return false;
                }
            } else {
                if (!$imgMedium->resizeFit($this->config['mediumWidth'], $this->config['mediumHeight'])) {
                    return false;
                }
            }
        }
        $options = array('file' => $medium);
        $medium = $imgMedium->output($type, $options);
        
        
        // Save thumbnail
        if (($imgThumb->width <= $this->config['thumbWidth']) && ($imgThumb->height <= $this->config['thumbHeight']) ) {
            if (in_array($type, array("gif", "jpeg", "png"))) {
                return true;
            }
        } else {
            if($this->config['resizeType'] == 'crop') {
                if (!$imgThumb->resizeCrop($this->config['thumbWidth'], $this->config['thumbHeight'])) {
                    return false;
                }
            } else {
                if (!$imgThumb->resizeFit($this->config['thumbWidth'], $this->config['thumbHeight'])) {
                    return false;
                }
            }
        }
        $options = array('file' => $thumb);
        $thumbnail = $imgThumb->output($type, $options);
