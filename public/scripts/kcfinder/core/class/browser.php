...
class browser extends uploader {
    ...
    public function __construct() {
        ...
        $mediumDir = $this->config['uploadDir'] . "/" . $this->config['mediumDir'];
        if (!$this->config['disabled'] &&
            ...
            (
                (
                !is_dir($mediumDir) &&
                !@mkdir($mediumDir, $this->config['dirPerms'])
                ) ||
            
                !is_readable($mediumDir) ||
                !dir::isWritable($mediumDir) ||
                (
                    !is_dir("$mediumDir/{$this->type}") &&
                    !@mkdir("$mediumDir/{$this->type}", $this->config['dirPerms'])
                )
            )
        )
        ...
        $this->mediumDir = $mediumDir;
        $this->mediumTypeDir = "$mediumDir/{$this->type}";
    ...
    protected function act_renameDir() {
        ...
        $mediumDir = "$this->mediumTypeDir/{$_POST['dir']}";
        if (is_dir($mediumDir))
            @rename($mediumDir, dirname($mediumDir) . "/$newName");
    ...
    protected function act_deleteDir() {
        ...
        $mediumDir = "$this->mediumTypeDir/{$_POST['dir']}";
        if (is_dir($mediumDir)) dir::prune($mediumDir);
        return true;
    }
    ...
    protected function act_rename() {
        ...
        $mediumDir = "{$this->mediumTypeDir}/{$_POST['dir']}";
        $mediumFile = "$mediumDir/{$_POST['file']}";
        
        if (file_exists($mediumFile))
            @rename($mediumFile, "$mediumDir/" . basename($newName));
        return true;
    }
    ...
    protected function act_delete() {
        ...
        $medium = "{$this->mediumTypeDir}/{$_POST['dir']}/{$_POST['file']}";
    ...
    protected function act_cp_cbd() {
        ...
        foreach($_POST['files'] as $file) {
            ...
            elseif (!@copy($path, "$dir/$base"))
                ...
                $fromMedium = "{$this->mediumDir}/$file";
                if (is_file($fromMedium) && is_readable($fromMedium)) {
                    $toMedium = "{$this->mediumTypeDir}/{$_POST['dir']}";
                    if (!is_dir($toMedium))
                        @mkdir($toMedium, $this->config['dirPerms'], true);
                        $toMedium .= "/$base";
                        @copy($fromMedium, $toMedium);
                }
            }
        }
        if (count($error))
    ...
    protected function moveUploadFile($file, $dir) {
        ...
        if($filesize > $this->config['maxFilesize']) {
            return $this->htmlData($file['name']) . ": " . $this->label("Kích thước phải nhỏ hơn ") . $this->config['maxFilesize'] .'KB';
        }
